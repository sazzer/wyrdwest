 ...
 */
function healthcheck(_: FastifyRequest<IncomingMessage>, reply: FastifyReply<ServerResponse>): void {
  reply.send({
    status: 'OK'
  });
}
 ...
 */
export function buildHealthcheckHandler(): ReadonlyArray<RouteOptions<Server, IncomingMessage, ServerResponse>> {
  return [
    {
      handler: healthcheck,
      method: 'GET',
